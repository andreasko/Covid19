...
function _fix_stan_data_types!(d)
    ...
    d["cases_start"] = Int(d["cases_start"])
...
#-----------------------------------------------------------------------------
path = defaultpath = "/home/and/code/ImperialCollegeReplications/FLA20a/Covid19/data/imperial-usa-cases/processed.rds"
function load_data(path = defaultpath)
    ...
    rename!(
        d,
        "f" => "π", "f2" => "π2", "SI" => "serial_intervals", "pop" => "population",
        ...
        "Pop_density" => "population_density", "W" => "num_weeks",
    )
    ...
    d["π2"] = collect(eachcol(d["π2"]))  # convert into Array of arrays instead of matrix
    ...
    turing_data = (; (k => d[String(k)] for k in [:num_states, :num_impute, :num_obs_states, :cases_start,
                                           :num_total_days, :cases, :deaths, :π, :π2, :epidemic_start,
